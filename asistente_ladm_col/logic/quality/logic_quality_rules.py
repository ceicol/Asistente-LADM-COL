...
class LogicQualityRules:
    ...
    def check_fdc_parcel_with_invalid_land_caterogy(self, db):
        ...
        res, records = self.get_ladm_queries(db.engine).get_invalid_null_values(db,
                                                                                db.names.FDC_PARCEL_T,
                                                                                db.names.FDC_PARCEL_T_LAND_CATEGORY_F)
        if res:
    ...
    def check_fdc_parcel_with_invalid_land_class(self, db):
        ...
        res, records = self.get_ladm_queries(db.engine).get_invalid_null_values(db,
                                                                                db.names.FDC_PARCEL_T,
                                                                                db.names.FDC_PARCEL_T_LAND_CLASS_F)
        if res:
    ...
    def check_fdc_parcel_with_invalid_economic_destination(self, db):
        ...
        res, records = self.get_ladm_queries(db.engine).get_invalid_null_values(db,
                                                                                db.names.FDC_PARCEL_T,
                                                                                db.names.FDC_PARCEL_T_ECONOMIC_DESTINATION_F)
        if res:
    ...
    def check_fdc_parcel_with_invalid_date_of_property_visit(self, db):
        ...
        res, records = self.get_ladm_queries(db.engine).get_invalid_null_values(db,
                                                                                db.names.FDC_PARCEL_T,
                                                                                db.names.FDC_PARCEL_T_DATE_OF_PROPERTY_VISIT_F)
        if res:
    ...
    def check_fdc_parcel_with_invalid_visit_result(self, db):
        ...
        res, records = self.get_ladm_queries(db.engine).get_invalid_null_values(db,
                                                                                db.names.FDC_PARCEL_T,
                                                                                db.names.FDC_PARCEL_T_VISIT_RESULT_F)
        if res:
    ...
    def check_fdc_parcel_with_invalid_has_register_area(self, db):
        ...
        res, records = self.get_ladm_queries(db.engine).get_invalid_null_values(db,
                                                                                db.names.FDC_PARCEL_T,
                                                                                db.names.FDC_PARCEL_T_HAS_REGISTRER_AREA_F)
        if res:
    ...
    def check_fdc_parcel_without_associated_right(self, db):
        rule = self.quality_rules_manager.get_quality_rule(EnumQualityRule.Logic.FDC_PARCEL_WITHOUT_ASSOCIATED_RIGHT)
        res, records = self.get_ladm_queries(db.engine).get_parcel_without_associated_right(db)
        if res:
            return self.basic_logic_validations(db, records, rule, QUALITY_RULE_ERROR_CODE_E422101)

    def check_fdc_right_without_associated_administrative_source(self, db):
        rule = self.quality_rules_manager.get_quality_rule(EnumQualityRule.Logic.FDC_RIGHT_WITHOUT_ASSOCIATED_ADMINISTRATIVE_SOURCE)
        res, records = self.get_ladm_queries(db.engine).get_right_without_associated_administrative_source(db)
        if res:
            return self.basic_logic_validations(db, records, rule, QUALITY_RULE_ERROR_CODE_E422201)

    def check_fdc_parcel_without_associated_plot(self, db):
        rule = self.quality_rules_manager.get_quality_rule(EnumQualityRule.Logic.FDC_PARCEL_WITHOUT_ASSOCIATED_PLOT)
