...
class GPKGLADMQuery(QGISLADMQuery):
    ...
    def get_right_with_invalid_right_type(db):
        query = """SELECT {t_id}, {t_ili_tid}
                   FROM {fdc_right_t}
                   WHERE {fdc_right_t_type_f}  = (select {t_id} from {fdc_right_t_type_d} where {ilicode} = '{fdc_right_type_without_right}')
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
    ...
    def get_parcel_without_associated_address(db):
        ...
        query = """SELECT {t_id}, {t_ili_tid}
                   FROM {fdc_parcel_t}
                   WHERE {t_id} NOT IN (
                        SELECT DISTINCT predio_direccion
                        FROM {ext_address_s} WHERE predio_direccion IS NOT NULL)
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
    ...
    def get_party_with_invalid_document_type(db):
        query = """SELECT {t_id}, {t_ili_tid}
                   FROM {fdc_party_t}
                   WHERE {fdc_party_t_document_type_f} = (select {t_id} from {fdc_party_document_type_d} where {ilicode} = '{fdc_party_document_type_sequential}')
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
    ...
    def get_parcel_without_associated_right(db):
        query = """SELECT {t_id}, {t_ili_tid}
                   ...
                   WHERE {t_id} NOT IN (
                        ...
                        FROM {fdc_right_t} WHERE {fdc_right_t_parcel_f} IS NOT NULL)
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
                            fdc_parcel_t=db.names.FDC_PARCEL_T,
    ...
    def get_right_without_associated_administrative_source(db):
        query = """SELECT {t_id}, {t_ili_tid}
                   ...
                   WHERE {t_id} NOT IN (
                        ...
                        FROM {fdc_administrative_source_right_t} WHERE {fdc_administrative_source_right_t_right_f} IS NOT NULL)
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
                            fdc_right_t=db.names.FDC_RIGHT_T,
    ...
    def get_parcel_without_associated_plot(db):
        query = """SELECT {t_id}, {t_ili_tid}
                   FROM {fdc_parcel_t}
                   WHERE {fdc_parcel_t_condition_f} != (SELECT {t_id} FROM {fdc_condition_parcel_type_d} WHERE {ilicode} LIKE '{parcel_type_horizontal_property_parcel_unit}')
                   AND {t_id} NOT IN (SELECT DISTINCT {fdc_plot_t_parcel_f} FROM {fdc_plot_t} WHERE {fdc_plot_t_parcel_f} IS NOT NULL)
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
                            fdc_parcel_t=db.names.FDC_PARCEL_T,
    ...
    def get_party_with_broken_relation_with_right(db):
        query = """SELECT {t_id}, {t_ili_tid}
                           FROM {fdc_party_t}
                           WHERE {t_id} NOT IN (
                                SELECT DISTINCT {fdc_right_t_party_f} FROM {fdc_right_t})
                         """.format(t_id=db.names.T_ID_F,
                                    t_ili_tid=db.names.T_ILI_TID_F,
                                    fdc_party_t=db.names.FDC_PARTY_T,
                                    fdc_right_t=db.names.FDC_RIGHT_T,
    ...
    def get_administrative_source_with_broken_relation_with_right(db):
        query = """SELECT {t_id}, {t_ili_tid}
                           FROM {fdc_administrative_source_t}
                           WHERE {t_id} NOT IN (
                                SELECT DISTINCT {fdc_administrative_source_right_t_administrative_source_f} FROM {fdc_administrative_source_right_t})
                         """.format(t_id=db.names.T_ID_F,
                                    t_ili_tid=db.names.T_ILI_TID_F,
    ...
    def get_fdc_housing_market_offers_with_broken_relation_with_parcel(db):
        query = """SELECT {t_id}, {t_ili_tid}
                           FROM {fdc_housing_market_offers_t}
    ...
    def get_fdc_building_unit_with_broken_relation_with_building(db):
        query = """SELECT {t_id}, {t_ili_tid}
                           FROM {fdc_building_unit_t}
                           WHERE {fdc_building_unit_t_building_f} NOT IN (
                                SELECT {t_id} FROM {fdc_building_t})
                         """.format(t_id=db.names.T_ID_F,
                                    t_ili_tid=db.names.T_ILI_TID_F,
                                    fdc_building_t=db.names.FDC_BUILDING_T,
