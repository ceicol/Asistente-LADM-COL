...
class PGLADMQuery(QGISLADMQuery):
        ...
        query = """
            ...
            FROM (
                   ...
                   WHERE ({lc_parcel_t_parcel_number_f} IS NOT NULL
                          AND (substring({lc_parcel_t_parcel_number_f},22,1) != '0'
                          AND {lc_parcel_t_parcel_type_f}=(select {t_id} from {schema}.{lc_condition_parcel_type_d} where {ilicode} = '{parcel_type_no_horizontal_property}'))
    ...
    def get_right_with_invalid_right_type(db):
        query = """SELECT {t_id}, {t_ili_tid}
                   ...
                   WHERE {fdc_right_t_type_f}  = (select {t_id} from {schema}.{fdc_right_type_d} where {ilicode} = '{fdc_right_type_without_right}')
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
                            schema=db.schema,
                            ...
                            fdc_right_type_d=db.names.FDC_RIGHT_TYPE_D,
    ...
    def get_parcel_without_associated_address(db):
        ...
        query = """SELECT {t_id}, {t_ili_tid}
                   ...
                   WHERE {t_id} NOT IN (
                        ...
                        FROM {schema}.{ext_address_s} WHERE predio_direccion IS NOT NULL)
                 """.format(t_id=db.names.T_ID_F,
                            t_ili_tid=db.names.T_ILI_TID_F,
                            ilicode=db.names.ILICODE_F,
                            schema=db.schema,
